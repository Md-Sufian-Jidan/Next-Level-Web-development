...
const ViewDoctorProfileDialog = ({
    ...
    // The "data" is an object that has a "data" property which is the actual data we want to display. So we need to access it using data.data
                        ...
                        {!isLoading && !isFetching && doctorDetails && (
                            <>
                                ...
                                <div className="rounded-lg border p-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {doctorDetails.specialties?.length ? (
                                            // Make a type and give in here. Because  "item" has "specialty" object inside it. So it's a nested object.
                                            doctorDetails.specialties.map((item: any) => (
                                ...
                                <div className="grid gap-4 md:grid-cols-2">
                                    <div className="rounded-lg border p-4">
                                        ...
                                        <div className="space-y-2">
                                            {/* Reviews is an array of objects. We need to map through it and display each review. */}
                                            {(doctorDetails.reviews ?? []).slice(0, 5).map((review: any, index: number) => (
                                    ...
                                    <div className="rounded-lg border p-4">
                                        ...
                                        <div className="space-y-2">
                                            {/* DoctorSchedules is an array of objects. Each object has a "schedule" object inside it. We need to map through it and display each schedule. */}
                                            {(doctorDetails.doctorSchedules ?? []).slice(0, 8).map((item: any, index: number) => (
                                ...
                                <div className="rounded-lg border p-4">
                                    ...
                                    <div className="space-y-2">
                                        {/* Appointments is an array of objects. Each object has a "patient" object inside it. We need to map through it and display each appointment. */}
                                        {(doctorDetails.appointments ?? []).slice(0, 10).map((appointment: any, index: number) => (
